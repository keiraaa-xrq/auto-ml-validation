...
class AutoFeatureSelector(BaseEstimator):
    ...
    def __init__(
        self,
        task: Literal["binary_classification", "multi_classification",
    ...
    def generate_best_feats(self, predictors: pd.DataFrame, target, model, scoring_method, n_jobs=-1, verbose=0) -> Tuple[List[str], int]:
        """
        ...
        if predictors.shape[0] <= 1:
            raise ValueError(
                "Number of samples too small, n_samples = {}".format(predictors.shape[0]))
        if not (len(predictors) == len(target)):
            raise ValueError(
                "Predictors dataframe and target dimension mismatch.")
        if not all(np.issubdtype(predictors[col].dtype, np.number) for col in predictors.columns):
            raise TypeError(
                "Object data found. Data needs to be processed to float or integer.")
        ...
        if self.method == 'intrinsic':
            feats_selected_, num_features = self._feature_select_intrinsic(
                predictors, target)
    ...
    def _feature_select_intrinsic(self, X: pd.DataFrame, y):
        ...
        def _log_reg_coefficients(X, y):
            sel_ = SelectFromModel(LogisticRegression(
                C=1000, penalty='l2', max_iter=500, random_state=10))
            sel_.fit(X, y)
    ...
    def _feature_select_greedy(self, X: pd.DataFrame, y, model, scoring_method, n_jobs, verbose=0):
        """
        ...
        def _forward_selected_feature(X, y, model, scoring_method, n_jobs, verbose=0):
            sfs1 = SFS(model,
        ...
        def _backward_selected_feature(X, y, model, scoring_method, n_jobs, verbose=0):
            sfs1 = SFS(model,
                       k_features=(1, len(X.columns)),
                       forward=False,
                       floating=False,
        ...
        def _exhausive_selected_feature(X, y, model, scoring_method, n_jobs):
            efs1 = EFS(model,
                       min_features=1,
        ...
        return _backward_selected_feature(X, y, model, scoring_method, n_jobs, verbose)
