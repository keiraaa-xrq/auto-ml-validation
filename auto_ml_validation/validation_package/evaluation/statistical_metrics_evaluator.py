...
class StatisticalMetricsEvaluator:
    ...
    def calculate_psi(self,
        ...
        grouped_train = self.processed_train.groupby(
            pd.cut(self.processed_train[score_col_name], bin_list)).count()
        grouped_train['perc_'+score_col_name] = grouped_train[score_col_name] / \
            sum(grouped_train[score_col_name])
        grouped_test = self.processed_test.groupby(
            pd.cut(self.processed_test[score_col_name], bin_list)).count()
        grouped_test['perc_'+score_col_name] = grouped_test[score_col_name] / \
            sum(grouped_test[score_col_name])
    ...
    def csi_for_each_feature(self,
        ...
        grouped_train = self.train.groupby(
            pd.cut(self.train[ft_name], bin_list)).count()
        grouped_train['perc_'+ft_name] = grouped_train[ft_name] / \
            sum(grouped_train[ft_name])
        grouped_test = self.test.groupby(
            pd.cut(self.test[ft_name], bin_list)).count()
        grouped_test['perc_'+ft_name] = grouped_test[ft_name] / \
            sum(grouped_test[ft_name])
