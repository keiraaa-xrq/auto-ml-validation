...
def generate_charts(results: Dict[str, Dict[str, any]], is_benchmark: bool):
    ...
    for dataset, result in results.items():
        ...
        for c in ["local_lime", "global_lime", "local_shap", "global_shap"]:
            result.get(c).savefig(f"{path}/result_images/{c}_{dataset}_{postfix}.png")
...
def generate_chart_table(doc, chart_name: str, results, include_bm: bool = True):
    generate_sub_heading(
        doc, interp_map.get(chart_name)["title"], interp_map.get(chart_name)["exp"]
    )
...
def generate_chart_auc_table(doc, chart_name: str, results, bm_results):
    ...
    for data, r in results.items():
        ...
        if bm_results:
            ...
            row[3].text = str(round(bm_results[data]["txt"]["auc"][auc_name], 6))
...
def generate_metric_table(doc, metric_name: str, results, bm_results):
    generate_sub_heading(
        doc, interp_map.get(metric_name)["title"], interp_map.get(metric_name)["exp"]
    )
    ...
    for data, r in results.items():
        ...
        if bm_results:
            row[2].text = str(bm_results.get(data)["txt"]["metrics"].get(metric_name))
...
def generate_psi_table(doc, results, bm_results):
    ...
    header[2].text = "PSI"
    cols = ["train_count", "train_perc", "test_count", "test_perc", "index_value"]
    for i in range(5):
...
def generate_csi_table(doc, results, bm_results):
    generate_sub_heading(
        doc, interp_map.get("csi")["title"], interp_map.get("csi")["exp"]
    )

    i = 0
    print(next(iter(results.items()))[1]['txt']['csi_dict'].keys())
    for f in next(iter(results.items()))[1]['txt']['csi_dict'].keys():
        ...
        for data, r in results.items():
            ...
            row_count = fill_stats_table(table, r["txt"]["csi_list"][i], row_count)

            if bm_results:
                row = table.rows[row_count].cells
                row[1].text = "Benchmark Model"
                row[2].text = str(round(bm_results[data]["txt"]["csi_dict"].get(f), 8))
                row_count = fill_stats_table(
                    table, bm_results[data]["txt"]["csi_list"][i], row_count
                )
...
def generate_ks_table(doc, results, bm_results):
    ...
    for data, r in results.items():
        ...
        if bm_results:
            row_count, row = add_row(table, row_count)
            row[1].text = "Benchmark Model"
            row[2].text = str(round(bm_results.get(data)["txt"]["ks"]["Train"], 8))
            row[3].text = str(round(bm_results.get(data)["txt"]["ks"]["Test"], 8))
            row[4].text = str(
...
def generate_feature_gini_table(doc, results, bm_results):
    ...
    generate_sub_heading(doc, interp_map.get('feature_gini')['title'], interp_map.get('feature_gini')['exp'])
    ...
    for data, r in results.items():
        ...
        row[0].text = data
        if bm_results:
            row_count = _fill_table(table, row_count, r['txt']['feature_gini'], bm_results[data]['txt']['feature_gini'])
        else:
...
def generate_gini_table(doc, results, bm_results):
    generate_sub_heading(doc, interp_map.get('dataset_gini')['title'], interp_map.get('dataset_gini')['exp'])
    cols = 3 if bm_results else 2
    ...
    for data, r in results.items():
        ...
        if bm_results:
            row[2].text = str(round(bm_results.get(data)['txt']['dataset_gini'], 8))
    doc.add_paragraph()